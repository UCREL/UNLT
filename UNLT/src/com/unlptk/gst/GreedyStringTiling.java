...
public class GreedyStringTiling {
	...
	public float generateTiles(ArrayList<String> srcTokens, ArrayList<String> susTokens, int minimalMatchingLength, float threshold){
		int initsearchSize =0;
		...
		int s = 0;
		String[] PList = new String[srcTokens.size()];
		PList = srcTokens.toArray(PList);
		
		String[] TList = new String[susTokens.size()];
		TList = susTokens.toArray(TList);
		...
		// Compute Similarity
		SimVal simResult = SimilarityCalculator.calcSimilarity(srcTokens, susTokens,tiles, threshold);
	 ...
	 * @return tiles
	 */
	public ArrayList<MatchVals> RKR_GST(String P, String T,int minimalMatchingLength, int initsearchSize) {
		if (minimalMatchingLength < 1)
			minimalMatchingLength = 3;
	 ...
	 * is returned, to be used to restart the scan pattern with it. All matches
